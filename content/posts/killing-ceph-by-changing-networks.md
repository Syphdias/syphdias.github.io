...
times — maybe this is this _Agile_ I heard so much about. But in all
...
I wouldn't even call it _changing a Monitor's IP_ since it basically adds a new
Ceph Mon with a new IP, wait for the Quorum, remove the old Ceph Mon.

The documentation only suggest to do this in a manual fashion but in practice
[cephadm] and [rook] should make this trivially easy…
...
says. Change my monmap but with two important changes:

- Using a volume for the `cephadm shell`
...
**Go to another terminal, back up the old monmap now!**

Now we remove the old IPs and add the new ones. I believe the up-to-date version
...
come up right away. I later [removed it][Ceph Mon Removal] after the Ceph Mons
were up again.

Since I did not want to do these steps for every Ceph Mon host, I copied the new
monmap to the relevant hosts, quick and dirty.
...
```bash
for i in {01..05}; do
    ssh ceph$i 'sudo cephadm shell --mount /root:/roothome --name mon.$HOSTNAME ceph-mon -i $HOSTNAME --inject-monmap /roothome/monmap-2025-11-05-new'
done
```

I updated `/var/lib/ceph/{FSID}/mon.{MON}/config` on every host. Either by
replacing the IPs in the config or by running `ceph config
generate-minimal-conf` in `cephadm shell`.
...
Note: For rolling back we would inject the old monmap for all Ceph Mons. After
...
In my case I had to `systemctl reset-failed 'ceph-*@mgr.ceph*'` and start the
...
automate it the first time and not the second time I had to do it.
